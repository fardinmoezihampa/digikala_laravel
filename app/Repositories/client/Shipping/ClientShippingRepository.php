...
class ClientShippingRepository implements ClientShippingRepositoryInterface
{
    public function checkQuantity($cartItems)
    {
        foreach ($cartItems as $item) {
            ...
            if ($product->stack > $item->quantity) {
                return back()->withErrors(['error' => " موجودی محصول {$product->name} کافی نیست."]);
            }
        }
    }

    public function createOrder($paymentMethodId, $orderNumber, $totalAmount, $deliveryMethodId, $addressId)
    {
    ...
    public function submitOrderBeforePayment($cartItems, $paymentMethodId, $orderNumber, $totalAmount, $deliveryMethodId, $addressId)
    {
        ...
        DB::transaction(function () use ($cartItems, $paymentMethodId, $orderNumber, $totalAmount, $deliveryMethodId, $addressId) {
            // insert to orders_table
            $order = $this->createOrder($paymentMethodId, $orderNumber, $totalAmount, $deliveryMethodId, $addressId);
            // insert to orderItems_table
            $this->createOrderItems($cartItems, $order->id);
            // insert to payments_table
